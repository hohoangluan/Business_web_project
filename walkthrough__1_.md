# 📘 HRMS – Project Walkthrough Toàn Diện

> **Hệ thống Quản lý Nhân sự (Human Resource Management System)**  
> Môn học: SE104 – Nhập môn Công nghệ Phần mềm  
> Stack: Django · SQLite3 · HTML/CSS/JS · Tailwind · Alpine.js · InsightFace

---

## 1. Tổng Quan Kiến Trúc

### 1.1 Tech Stack

| Tầng | Công nghệ |
|------|-----------|
| **Backend** | Django (Python) — MTV pattern |
| **Database** | SQLite3 (dev) → MySQL (production) |
| **Frontend** | HTML/CSS, Tailwind CSS, Alpine.js |
| **AI** | InsightFace (face recognition cho chấm công) |
| **Email** | Gmail SMTP (OTP đặt lại mật khẩu) |
| **Kiến trúc** | Client–Server · Multi-App Django · 3 lớp |

> [!NOTE]
> Các tính năng sau **đã bị loại bỏ** khỏi phạm vi hiện tại: CEO role, Super_User role, Audit Log, Cơ chế tính lương (BangLuong).

### 1.2 Cấu Trúc 3 Lớp

```
┌─────────────────────────────────────┐
│  Presentation Layer                 │  Templates (HTML/CSS/JS/Tailwind)
│  → Views, Forms, URL routing        │
├─────────────────────────────────────┤
│  Business Logic Layer               │  Services, validation, workflows
│  → services/, forms validation      │
├─────────────────────────────────────┤
│  Data Layer                         │  Models, Django ORM
│  → models/, migrations/             │
└─────────────────────────────────────┘
```

### 1.3 Cấu Trúc App (10 Django Apps)

```
business_web/               ← Project root (settings, urls, wsgi)
├── accounts/               ← Tài khoản, đăng nhập, phân quyền, RBAC
├── employee_profiles/      ← Hồ sơ nhân sự (thông tin cá nhân, công việc)
├── contracts/              ← Hợp đồng lao động, cảnh báo hết hạn
├── attendance/             ← Chấm công FaceID, bảng công, điều chỉnh
├── leaves/                 ← Nghỉ phép, quỹ phép
├── overtime/               ← Tăng ca (OT), phê duyệt OT
├── performance/            ← Đánh giá nhân viên định kỳ
├── rewards_discipline/     ← Khen thưởng, xử phạt
├── reports_interactions/   ← Báo cáo công việc, helpdesk ticket
└── stats_reports/          ← Thống kê tổng hợp (không có model riêng)
```

---

## 2. Data Models

### 2.1 App: `accounts` — Tài Khoản & Phân Quyền

#### `Role`
| Trường | Kiểu | Mô tả |
|--------|------|-------|
| `name` | CharField (unique) | Tên vai trò: `admin` \| `hr` \| `manager` \| `leader` \| `employee` |
| `description` | TextField | Mô tả vai trò |

> [!IMPORTANT]
> Chỉ có **5 vai trò** trong code thực tế. Không có `CEO` hay `Super_User`.

#### `CustomPermission`
| Trường | Kiểu | Mô tả |
|--------|------|-------|
| `codename` | CharField (unique) | Mã quyền hạn |
| `name` | CharField | Tên hiển thị |

#### `UserProfile`
> **1 User Django ↔ 1 UserProfile** (OneToOne)

| Trường | Kiểu | Mô tả |
|--------|------|-------|
| `user` | OneToOneField → User | Django User sở hữu profile |
| `role` | ForeignKey → Role | Vai trò hệ thống (RBAC) |
| `permissions` | ManyToManyField → CustomPermission | Quyền riêng lẻ |
| `full_name` | CharField | Họ tên đầy đủ |
| `employee_id` | CharField (unique) | MSNV — format: `[YY][MaPhongBan][STT4]` |

#### `OtpCode`
| Trường | Kiểu | Mô tả |
|--------|------|-------|
| `user` | ForeignKey → User | User yêu cầu reset mật khẩu |
| `code` | CharField(6) | Mã OTP 6 chữ số |
| `created_at` | DateTimeField | Thời điểm tạo — hết hạn sau **120 giây** |

---

### 2.2 App: `employee_profiles` — Hồ Sơ Nhân Sự

#### `PersonalInfo`
> **1 User ↔ 1 PersonalInfo** (OneToOne, `related_name='personal_info'`)

| Trường | Kiểu | Mô tả |
|--------|------|-------|
| `user` | OneToOneField → User | Chủ sở hữu |
| `phone_number` | CharField(20) | Số điện thoại (10 chữ số) |
| `date_of_birth` | CharField(10) | Ngày sinh DD/MM/YYYY |
| `gender` | CharField | Giới tính |
| `marital_status` | CharField | Tình trạng hôn nhân |
| `nationality` | CharField | Quốc tịch |
| `id_card_number` | CharField | Số CCCD/CMND |
| `id_card_issue_place` | CharField | Nơi cấp |
| `id_card_issue_date` | CharField | Ngày cấp |
| `permanent_address` | TextField | Địa chỉ thường trú |
| `temporary_address` | TextField | Địa chỉ tạm trú |

#### `EmployeeWorkInfo`
> **1 User ↔ 1 EmployeeWorkInfo** (OneToOne, `related_name='work_info'`)

| Trường | Kiểu | Mô tả |
|--------|------|-------|
| `user` | OneToOneField → User | Chủ sở hữu |
| `employee_type` | CharField | Loại NV: Toàn thời gian, Bán thời gian, Thực tập |
| `department` | CharField | Phòng ban |
| `position` | CharField | Chức danh |
| `workplace` | CharField | Nơi làm việc |
| `probation_start` | CharField | Ngày bắt đầu thử việc |
| `official_start_date` | CharField | Ngày làm việc chính thức |
| `work_status` | CharField (choices) | `working`, `probation`, `paused`, `resigned` |
| `manager_user` | ForeignKey → User | Quản lý trực tiếp |
| `leader_user` | ForeignKey → User | Leader phụ trách |

#### `EducationAndSkills`
> **1 User ↔ 1 EducationAndSkills** (OneToOne)

| Trường | Kiểu | Mô tả |
|--------|------|-------|
| `education_level` | CharField | Trình độ học vấn |
| `degree` | CharField | Bằng cấp |
| `major` | CharField | Chuyên ngành |
| `certificates` | TextField | Chứng chỉ |
| `foreign_languages` | TextField | Ngoại ngữ |
| `professional_skills` | TextField | Kỹ năng chuyên môn |

#### `EmergencyContact`
| Trường | Kiểu | Mô tả |
|--------|------|-------|
| `user` | OneToOneField → User | Nhân viên |
| `contact_name` | CharField | Tên người liên hệ khẩn |
| `relation` | CharField | Mối quan hệ |
| `contact_phone` | CharField | Số điện thoại khẩn |
| `contact_address` | TextField | Địa chỉ người liên hệ |

#### `EmployeeDocument`
| Trường | Kiểu | Mô tả |
|--------|------|-------|
| `user` | ForeignKey → User | Nhân viên sở hữu |
| `document_type` | CharField | Loại tài liệu |
| `file` | FileField | File đính kèm |

---

### 2.3 App: `contracts` — Hợp Đồng Lao Động

#### `ContractInfo`
> **1 User ↔ N ContractInfo** (ForeignKey) — mỗi NV có thể ký nhiều HĐ theo thời gian; tại một thời điểm chỉ có **1 HĐ đang hiệu lực** (`QĐ_HieuLuc`).

| Trường | Kiểu | Mô tả |
|--------|------|-------|
| `user` | ForeignKey → User | Nhân viên có hợp đồng |
| `is_active` | BooleanField (default=True) | HĐ đang hiệu lực? (unique=True khi True để đảm bảo QĐ_HieuLuc) |
| `contract_number` | CharField | Số HĐ (unique) — VD: `HD-2026-001` |
| `contract_type` | CharField | Loại HĐ — VD: Thử việc, Chính thức 1 năm |
| `contract_signed_date` | CharField | Ngày ký (DD/MM/YYYY) |
| `contract_start_date` | CharField | Ngày hiệu lực |
| `contract_end_date` | CharField | Ngày hết hạn (để trống = không thời hạn) |
| `contract_annual_leave_days` | PositiveIntegerField | Ngày phép/năm theo HĐ |
| `contract_standard_shift` | CharField | Ca làm tiêu chuẩn — VD: 08:30-17:30 |
| `contract_attachment_reference` | CharField | Tên/link file PDF hợp đồng |

> **Ràng buộc nghiệp vụ:** `ngayBatDau >= ngayKy`, `ngayHetHan >= ngayBatDau`, mỗi NV chỉ có 1 HĐ `is_active=True`

---

### 2.4 App: `attendance` — Chấm Công

#### `EmployeeFace`
> **1 User ↔ 1 EmployeeFace** (OneToOne)

| Trường | Kiểu | Mô tả |
|--------|------|-------|
| `user` | OneToOneField → User | Nhân viên sở hữu |
| `face_base64` | TextField | Ảnh khuôn mặt mã hóa Base64 |
| `slot_id` | PositiveSmallIntegerField | Slot FastAPI (mặc định 1, hỗ trợ tối đa 5) |
| `embedding` | JSONField | Vector 512 chiều từ InsightFace |
| `content_type` | CharField | MIME type ảnh: `image/png`, `image/jpeg` |
| `updated_at` | DateTimeField | Lần cập nhật cuối |

#### `AttendanceRecord`
> **1 User ↔ N AttendanceRecord** (unique_together: `user + record_date`)

| Trường | Kiểu | Mô tả |
|--------|------|-------|
| `user` | ForeignKey → User | Nhân viên chấm công |
| `record_date` | DateField | Ngày chấm công |
| `check_in_time` | TimeField (nullable) | Giờ vào làm |
| `check_out_time` | TimeField (nullable) | Giờ tan làm |
| `status` | CharField | `on_time`, `late`, `early_leave`, `absent` |

#### `AttendanceAdjustmentRequest`
> **1 AttendanceRecord ↔ 1 AdjustmentRequest** (OneToOne)

| Trường | Kiểu | Mô tả |
|--------|------|-------|
| `record` | OneToOneField → AttendanceRecord | Bản ghi cần điều chỉnh |
| `submitted_by` | ForeignKey → User | Nhân viên nộp yêu cầu |
| `reason` | CharField (choices) | `forgot`, `technical`, `business_trip`, `other` |
| `reason_detail` | TextField | Chi tiết lý do |
| `claimed_check_out_time` | TimeField | Giờ ra khai báo thực tế |
| `evidence` | FileField | Ảnh/PDF minh chứng |
| `status` | CharField | `pending`, `approved`, `rejected` |
| `reviewed_by` | ForeignKey → User | HR duyệt |
| `hr_note` | TextField | Ghi chú từ HR |

---

### 2.5 App: `leaves` — Nghỉ Phép

#### `LeaveRequest`

| Trường | Kiểu | Mô tả |
|--------|------|-------|
| `user` | ForeignKey → User | Nhân viên gửi đơn |
| `leave_type` | CharField (choices) | `annual`, `sick`, `personal`, `maternity`, `business`, `other` |
| `start_date` | DateField | Ngày bắt đầu nghỉ |
| `end_date` | DateField | Ngày kết thúc |
| `days` | DecimalField(4,1) | Số ngày nghỉ |
| `reason` | TextField | Lý do |
| `status` | CharField (choices) | `pending` → `leader_approved` → `approved` / `rejected` |
| `leader_approved_by` | ForeignKey → User | Leader/Manager duyệt L1 |
| `leader_approved_at` | DateTimeField | Thời điểm duyệt L1 |
| `approved_by` | ForeignKey → User | HR duyệt L2 |
| `rejected_reason` | TextField | Lý do từ chối |
| `attachment` | FileField (nullable) | Tệp minh chứng đính kèm (PDF/JPG/PNG, ≤5MB) |

> **Quy tắc L1:** Ai được gán là `leader_user` **hoặc** `manager_user` của NV trong `EmployeeWorkInfo` thì người đó duyệt — không phân biệt theo số ngày nghỉ.

---

### 2.6 App: `overtime` — Tăng Ca

#### `OvertimeRequest`

| Trường | Kiểu | Mô tả |
|--------|------|-------|
| `user` | ForeignKey → User | Nhân viên đăng ký |
| `overtime_date` | DateField | Ngày tăng ca |
| `start_time` | TimeField | Giờ bắt đầu OT |
| `end_time` | TimeField | Giờ kết thúc OT |
| `hours` | DecimalField(4,1) | Số giờ OT |
| `reason` | TextField | Lý do |
| `status` | CharField | `pending` → `leader_approved` → `approved` / `rejected` |
| `leader_approved_by` | ForeignKey → User | Leader/Manager duyệt L1 |
| `leader_approved_at` | DateTimeField | Thời điểm duyệt L1 |
| `approved_by` | ForeignKey → User | HR duyệt L2 |
| `rejected_reason` | TextField | Lý do từ chối |
| `attachment` | FileField (nullable) | Tệp minh chứng đính kèm (nếu có) |

> **Quy tắc L1:** Ai được gán là `leader_user` **hoặc** `manager_user` của NV trong `EmployeeWorkInfo` thì người đó duyệt — không phân biệt theo số giờ OT.

---

### 2.7 App: `performance` — Đánh Giá

#### `EvaluationCategory`
| Trường | Kiểu | Mô tả |
|--------|------|-------|
| `name` | CharField (unique) | Tên loại: Chuyên cần, Hiệu suất, Kỹ năng nhóm... |
| `description` | TextField | Mô tả loại đánh giá |

#### `Evaluation`

| Trường | Kiểu | Mô tả |
|--------|------|-------|
| `employee` | ForeignKey → User | Nhân viên được đánh giá |
| `reviewer` | ForeignKey → User | Manager/Leader thực hiện đánh giá |
| `category` | ForeignKey → EvaluationCategory | Loại đánh giá |
| `status` | CharField | `draft` → `submitted` → `acknowledged` |
| `rating` | CharField (choices) | `A` (Xuất sắc ≥90), `B` (Tốt ≥75), `C` (TB ≥60), `D` (Cần cải thiện <60) — **tự động tính từ `score`**, không nhập tay |
| `score` | PositiveSmallIntegerField (0–100) | Điểm đánh giá thang 100 |
| `evaluation_date` | DateField | Ngày đánh giá |
| `content` | TextField | Nội dung đánh giá |
| `evidence_reference` | CharField | File minh chứng |
| `acknowledged_by` | ForeignKey → User | HR xác nhận |
| `acknowledged_at` | DateTimeField | Thời điểm HR xác nhận |
| `hr_note` | TextField | Ghi chú phản hồi từ HR |

> **Quy định:** Sau khi `submitted` → KHÔNG thể chỉnh sửa

---

### 2.8 App: `rewards_discipline` — Khen Thưởng / Xử Phạt

#### `RewardPenalty`

| Trường | Kiểu | Mô tả |
|--------|------|-------|
| `employee` | ForeignKey → User | Nhân viên được thưởng/phạt |
| `record_type` | CharField | `reward` (Khen thưởng) / `penalty` (Xử phạt) |
| `amount` | PositiveIntegerField | Số tiền (VND), 0 = văn bản |
| `reason_title` | CharField | Tiêu đề lý do |
| `reason_detail` | TextField | Chi tiết lý do |
| `proposer` | ForeignKey → User | Người đề xuất (Leader/Manager) |
| `status` | CharField | `pending` → `approved` / `rejected` |
| `application_date` | DateField | Ngày áp dụng |
| `evidence_file` | FileField | File minh chứng |

---

### 2.9 App: `reports_interactions` — Báo Cáo & Helpdesk

#### `Report`

| Trường | Kiểu | Mô tả |
|--------|------|-------|
| `author` | ForeignKey → User | Người gửi báo cáo |
| `recipient` | ForeignKey → User | Quản lý nhận báo cáo |
| `title` | CharField | Tiêu đề |
| `content` | TextField | Nội dung |
| `file_attachment` | FileField | File đính kèm |
| `is_viewed` | BooleanField | Quản lý đã xem chưa |
| `viewed_at` | DateTimeField | Thời điểm quản lý xem |
| `status` | CharField (choices) | `submitted` → `needs_update` → `acknowledged` (hỗ trợ luồng "Yêu cầu cập nhật") |
| `manager_note` | TextField (nullable) | Nội dung phản hồi / chỉ đạo của quản lý |

> **Quy định:** Sau khi `status = acknowledged` → người gửi KHÔNG thể sửa/xóa. `is_viewed` được set khi Manager mở xem; `status` mới là trạng thái nghiệp vụ chính thức.

#### `Ticket`

| Trường | Kiểu | Mô tả |
|--------|------|-------|
| `author` | ForeignKey → User | Người tạo ticket |
| `ticket_type` | CharField | `support` (Hỗ trợ) / `complaint` (Khiếu nại) |
| `priority` | CharField | `low`, `medium`, `high` |
| `title` | CharField | Tiêu đề |
| `content` | TextField | Nội dung chi tiết |
| `evidence_file` | FileField | File minh chứng |
| `status` | CharField | `new` → `processing` → `resolved` → `closed` / `rejected` |
| `assigned_to` | ForeignKey → User | Người xử lý (được định tuyến tự động) |
| `rejection_reason` | TextField | Lý do từ chối |

---

### 2.10 App: `stats_reports` — Thống Kê Tổng Hợp

> App này **không có model riêng**. Đọc và tổng hợp dữ liệu từ các app: `attendance`, `leaves`, `overtime`, `performance`, `rewards_discipline`.

---

## 3. Sơ Đồ Quan Hệ Model (ERD Tổng Quan)

```mermaid
erDiagram
    User ||--|| UserProfile : "has"
    User ||--|| PersonalInfo : "has"
    User ||--|| EmployeeWorkInfo : "has"
    User ||--|| EducationAndSkills : "has"
    User ||--|| EmergencyContact : "has"
    User ||--o{ ContractInfo : "signs (nhiều HĐ theo thời gian)"
    User ||--|| EmployeeFace : "has"
    User ||--o{ EmployeeDocument : "uploads"
    User ||--o{ AttendanceRecord : "clocks"
    User ||--o{ LeaveRequest : "submits"
    User ||--o{ OvertimeRequest : "registers"
    User ||--o{ OtpCode : "requests"
    User ||--o{ RewardPenalty : "receives"
    User ||--o{ Evaluation : "receives (employee)"
    User ||--o{ Evaluation : "creates (reviewer)"
    User ||--o{ Report : "sends"
    User ||--o{ Ticket : "creates"
    UserProfile }o--|| Role : "assigned"
    UserProfile }o--o{ CustomPermission : "granted"
    AttendanceRecord ||--o| AttendanceAdjustmentRequest : "may have"
    EvaluationCategory ||--o{ Evaluation : "categorizes"
    EmployeeWorkInfo }o--|| User : "manager_user"
    EmployeeWorkInfo }o--|| User : "leader_user"
```

---

## 4. Ma Trận Phân Quyền (RBAC)

> [!NOTE]
> Hệ thống chỉ có **5 vai trò** được định nghĩa trong `Role` model: `admin`, `hr`, `manager`, `leader`, `employee`.

| Chức năng | Admin | HR | Manager | Leader | Employee |
|-----------|:---:|:---:|:---:|:---:|:---:|
| Quản lý tài khoản (gán role, khóa/mở, xóa, reset MK) | ✅ | | | | |
| Toàn quyền hồ sơ & hợp đồng | | ✅ | | | |
| Phê duyệt L2 (nghỉ phép, OT, thưởng/phạt) | | ✅ | | | |
| Điều chỉnh giờ công thủ công | | ✅ | | | |
| Phê duyệt L1 — nếu là `manager_user` của NV đó | | | ✅ | | |
| Phê duyệt L1 — nếu là `leader_user` của NV đó | | | | ✅ | |
| Đánh giá nhân viên, lập phiếu thưởng/phạt | | | ✅ | ✅ | |
| Xem/nộp dữ liệu của chính mình | | | | | ✅ |

> [!IMPORTANT]
> **Quy tắc L1 thực tế:** Cả Leader và Manager đều có thể duyệt L1. Điều kiện không phụ thuộc vào số ngày/số giờ, mà chỉ kiểm tra: `approver in (employee.work_info.leader_user, employee.work_info.manager_user)`. Ai được gán là supervisor trực tiếp của NV đó → người đó duyệt.

---

## 5. Workflow & Sequence Diagrams

### 5.1 Đăng Nhập & Bảo Mật

```mermaid
sequenceDiagram
    actor User as Nhân viên
    participant Web as Web Browser
    participant Auth as accounts/views/auth
    participant DB as Database
    participant Session as Django Session

    User->>Web: Nhập username + password
    Web->>Auth: POST /login/
    Auth->>DB: Tìm User theo username
    DB-->>Auth: User record

    alt Tài khoản bị khóa (is_active=False)
        Auth-->>Web: ❌ "Tài khoản bị khóa"
    else Mật khẩu sai
        Auth->>DB: Tăng failed_login_count
        alt failed_count >= 3
            Auth->>DB: Khóa tài khoản (is_active=False)
            Auth-->>Web: ❌ "Tài khoản bị khóa do sai 3 lần"
        else
            Auth-->>Web: ❌ "Sai mật khẩu"
        end
    else Đăng nhập thành công
        Auth->>DB: Reset failed_login_count = 0
        Auth->>Session: Tạo session, ghi role
        Auth-->>Web: ✅ Redirect Dashboard theo Role
    end
```

---

### 5.2 Quên Mật Khẩu (OTP qua Email)

```mermaid
sequenceDiagram
    actor User as Nhân viên
    participant Web as Browser
    participant View as forgot_password_view
    participant DB as Database
    participant SMTP as Gmail SMTP

    User->>Web: Nhập email
    Web->>View: POST /forgot-password/
    View->>DB: Tìm User theo email
    DB-->>View: User record

    alt Email không tồn tại
        View-->>Web: ❌ "Email không tìm thấy"
    else Email hợp lệ
        View->>View: Sinh mã OTP 6 chữ số
        View->>DB: Lưu OtpCode(user, code, created_at)
        View->>SMTP: Gửi email chứa OTP
        SMTP-->>User: 📧 Email OTP
        View-->>Web: "Kiểm tra email của bạn"
        User->>Web: Nhập OTP + mật khẩu mới
        Web->>View: POST /reset-password/
        View->>DB: Lấy OtpCode mới nhất của user
        alt OTP hết hạn (> 120 giây)
            View-->>Web: ❌ "OTP đã hết hạn"
        else OTP sai
            View-->>Web: ❌ "Mã OTP không đúng"
        else OTP đúng & còn hạn
            View->>DB: Cập nhật password mới
            View->>DB: Xóa OtpCode
            View-->>Web: ✅ "Đổi mật khẩu thành công"
        end
    end
```

---

### 5.3 Tạo Hồ Sơ Nhân Viên Mới (HR)

```mermaid
sequenceDiagram
    actor HR as HR Staff
    participant Web as Browser
    participant View as employee_profiles/views
    participant Service as services/
    participant DB as Database
    participant Email as Gmail SMTP

    HR->>Web: Điền form hồ sơ (BM1→BM7) + upload ảnh khuôn mặt
    Web->>View: POST /employees/create/
    View->>View: Validate dữ liệu (tuổi>=18, SĐT 10 số, CCCD, ảnh 3x4)

    alt Validation failed
        View-->>Web: ❌ Hiển thị lỗi form
    else Validation passed
        View->>Service: Gọi generate_employee_id(department)
        Service-->>View: MSNV = [YY][MaPhongBan][STT4]
        View->>Service: Gọi generate_username(ten, ho, phongban)
        Service-->>View: Username = [ten][ho][maphongban] (viết thường không dấu)
        View->>DB: Tạo User(username, password ngẫu nhiên)
        View->>DB: Tạo UserProfile(user, employee_id, role=Employee)
        View->>DB: Tạo PersonalInfo(user, ...)
        View->>DB: Tạo EmployeeWorkInfo(user, department, manager, leader...)
        View->>DB: Tạo ContractInfo(user, ...)
        View->>DB: Tạo EmployeeFace(user, face_base64, embedding)
        View->>Email: Gửi email thông báo tài khoản (username, password)
        View-->>Web: ✅ "Tạo hồ sơ thành công"
    end
```

---

### 5.4 Chấm Công FaceID

```mermaid
sequenceDiagram
    actor Employee as Nhân viên
    participant Web as Browser (Webcam)
    participant View as attendance/views
    participant FaceService as InsightFace Service
    participant DB as Database

    Employee->>Web: Nhấn Check-in / Check-out
    Web->>Web: Webcam chụp ảnh + lấy IP address
    Web->>View: POST /attendance/checkin/ (ảnh base64, IP)

    View->>View: Kiểm tra IP có trong whitelist?
    alt IP không hợp lệ
        View-->>Web: ❌ "Không được phép chấm công từ địa chỉ này"
    else IP hợp lệ
        View->>FaceService: Trích xuất face embedding từ ảnh
        FaceService-->>View: embedding vector (512 chiều)

        View->>DB: Lấy EmployeeFace của user đang đăng nhập
        DB-->>View: embedding mẫu đã lưu

        View->>View: So khớp cosine similarity (threshold ~0.5)
        alt Không khớp khuôn mặt
            View-->>Web: ❌ "Không nhận diện được khuôn mặt"
        else Khớp khuôn mặt
            View->>DB: Lấy/Tạo AttendanceRecord(user, today)
            alt Check-in
                View->>View: Xác định trạng thái
                Note over View: check_in < 08:00 → on_time<br/>check_in > 08:00 → late
                View->>DB: Lưu check_in_time, status
                View-->>Web: ✅ "Check-in thành công lúc HH:MM"
            else Check-out
                View->>View: Kiểm tra giờ về sớm
                Note over View: check_out < giờ_kết_thúc_ca → early_leave
                View->>DB: Lưu check_out_time
                View-->>Web: ✅ "Check-out thành công lúc HH:MM"
            end
        end
    end
```

---

### 5.5 Nghỉ Phép — Luồng Phê Duyệt 2 Cấp

```mermaid
sequenceDiagram
    actor Employee as Nhân viên
    actor L1 as Leader / Manager (L1)
    actor HR as HR (L2)
    participant System as Hệ thống
    participant DB as Database
    participant Notif as Notification

    Employee->>System: Nộp đơn nghỉ phép (loại, ngày, lý do)
    System->>DB: Kiểm tra số ngày <= quỹ phép còn lại?
    alt Vượt quỹ phép
        System-->>Employee: ❌ "Không đủ phép"
    else Hợp lệ
        System->>DB: Tạo LeaveRequest(status=pending)
        System->>Notif: Thông báo cho supervisor trực tiếp của NV

        Note over L1: L1 = leader_user hoặc manager_user<br/>được gán trong EmployeeWorkInfo của NV

        L1->>System: Duyệt L1 (hoặc Từ chối)
        alt L1 Từ chối
            System->>DB: status = rejected
            System->>Notif: Thông báo NV bị từ chối
        else L1 Duyệt
            System->>DB: status = leader_approved, ghi leader_approved_by & at
            System->>Notif: Thông báo HR cần duyệt L2

            HR->>System: Duyệt L2 (hoặc Từ chối)
            alt L2 Từ chối
                System->>DB: status = rejected
                System->>Notif: Thông báo NV
            else L2 Duyệt
                System->>DB: status = approved, ghi approved_by
                System->>DB: Trừ quỹ phép của NV
                System->>Notif: ✅ Thông báo NV được duyệt phép
            end
        end
    end
```

---

### 5.6 Tăng Ca OT — Luồng Phê Duyệt 2 Cấp

```mermaid
sequenceDiagram
    actor Employee as Nhân viên
    actor L1 as Leader / Manager (L1)
    actor HR as HR (L2)
    participant System as Hệ thống
    participant DB as Database

    Employee->>System: Đăng ký OT (ngày, giờ bắt đầu/kết thúc, lý do)
    System->>DB: Tạo OvertimeRequest(status=pending)

    Note over L1: L1 = leader_user hoặc manager_user<br/>được gán trong EmployeeWorkInfo của NV

    L1->>System: Duyệt L1 / Từ chối
    alt Từ chối
        System->>DB: status = rejected
    else Duyệt
        System->>DB: status = leader_approved, ghi leader_approved_by & at
        Note over System: Ngoại lệ: NV có role HR<br/>→ approved luôn, bỏ qua L2
        HR->>System: Duyệt L2 / Từ chối
        alt Duyệt
            System->>DB: status = approved
        end
    end
```

---

### 5.7 Hợp Đồng — Cảnh Báo Tự Động (Batch Job)

```mermaid
sequenceDiagram
    participant Cron as Batch Job (1-3h AM mỗi ngày)
    participant DB as Database
    participant Notif as Notification System
    actor HR as HR
    actor Manager as Manager
    actor Employee as Nhân viên

    Cron->>Cron: Trigger lúc 1-3h AM
    Cron->>DB: Lấy tất cả ContractInfo có contract_end_date != null
    DB-->>Cron: Danh sách hợp đồng

    loop Mỗi hợp đồng
        Cron->>Cron: Tính số ngày còn lại = end_date - today

        alt Còn 30 ngày (và chưa gửi cảnh báo 30 ngày)
            Cron->>Notif: Gửi thông báo cho HR + Manager
            Cron->>DB: Cập nhật LanCanhBaoCuoi = 30d
        else Còn 15 ngày
            Cron->>Notif: Gửi thêm thông báo cho Employee
            Cron->>DB: Cập nhật LanCanhBaoCuoi = 15d
        else Còn 7 ngày
            Cron->>Notif: Gửi thông báo KHẨN cho tất cả (HR + Manager + Employee)
            Cron->>DB: Cập nhật LanCanhBaoCuoi = 7d
        else Đã hết hạn
            Cron->>DB: Cập nhật status = "Hết hiệu lực"
            Cron->>Notif: Gửi thông báo KHẨN cho HR
        end
    end
```

---

### 5.8 Đánh Giá Nhân Viên

```mermaid
sequenceDiagram
    actor Reviewer as Leader / Manager
    actor HR as HR
    participant System as Hệ thống
    participant DB as Database

    Reviewer->>System: Tạo phiếu đánh giá (nháp)
    System->>DB: Tạo Evaluation(status=draft, reviewer, employee, category)
    Reviewer->>System: Nhập xếp loại (A/B/C/D), nội dung + minh chứng
    Reviewer->>System: Nhấn "Gửi"

    System->>DB: status = submitted
    Note over DB: 🔒 Khóa chỉnh sửa vĩnh viễn

    System->>HR: Thông báo có đánh giá mới cần xác nhận
    HR->>System: Xem đánh giá, thêm ghi chú (hr_note)
    HR->>System: Xác nhận

    System->>DB: status = acknowledged, ghi acknowledged_by & at
```

---

### 5.9 Khen Thưởng / Xử Phạt — Luồng 2 Cấp

```mermaid
sequenceDiagram
    actor Leader as Leader / Manager
    actor Manager as Manager (L1 nếu Leader lập)
    actor HR as HR (L2)
    participant System as Hệ thống
    participant DB as Database

    Leader->>System: Lập phiếu thưởng/phạt (BM13/BM14)
    System->>DB: Tạo RewardPenalty(status=pending, proposer=Leader)

    alt Leader lập phiếu
        System->>Manager: Thông báo cần duyệt L1
        Manager->>System: Duyệt L1 / Từ chối
        alt Từ chối
            System->>DB: status = rejected
            Note over System: Timeout 48h → tự động hủy
        else Duyệt L1
            System->>HR: Thông báo cần duyệt L2
        end
    else Manager lập phiếu
        System->>HR: Thông báo cần duyệt L2 (bỏ qua L1)
    end

    HR->>System: Duyệt L2 / Từ chối
    alt Duyệt
        System->>DB: status = approved
        System->>DB: Cập nhật amount vào RewardPenalty record
    else Từ chối
        System->>DB: status = rejected
    end
```

---

### 5.10 Báo Cáo Công Việc

```mermaid
sequenceDiagram
    actor Employee as Nhân viên / Leader
    actor Manager as Manager / HR
    participant System as Hệ thống
    participant DB as Database

    Note over Employee: Employee: nộp trước Thứ 6 17:00 (gửi cho Leader)<br/>Leader: nộp trước Thứ 2 9:00 (gửi cho Manager)<br/>Manager và HR không có nghĩa vụ nộp báo cáo

    Employee->>System: Tạo và gửi báo cáo (tiêu đề, nội dung, file đính kèm)
    System->>DB: Tạo Report(author=Employee, recipient=Manager, is_viewed=False)
    System->>Manager: Thông báo có báo cáo mới

    Manager->>System: Xem báo cáo
    System->>DB: is_viewed = True, viewed_at = now()

    Note over DB: 🔒 Khóa chỉnh sửa — Employee không thể sửa/xóa

    Manager->>System: "Xác nhận tiếp nhận"
    Note over Manager: Báo cáo là đầu vào<br/>quan trọng cho đánh giá định kỳ
```

---

### 5.11 Helpdesk Ticket — Định Tuyến Tự Động

```mermaid
sequenceDiagram
    actor Employee as Nhân viên
    participant System as Hệ thống
    participant Router as Auto-Router
    participant Assignee as Người xử lý
    participant DB as Database

    Employee->>System: Tạo ticket (loại, nhóm, tiêu đề, nội dung, file)
    System->>Router: Xác định bộ phận xử lý theo loại yêu cầu

    alt Loại: IT / Kỹ thuật
        Router->>DB: assigned_to = Admin
    else Loại: Lương / Phép / Hành chính
        Router->>DB: assigned_to = HR
    else Loại: Kết quả đánh giá
        Router->>DB: assigned_to = Manager
    else Loại: CSVC
        Router->>DB: assigned_to = Hành chính
    else Loại: Môi trường làm việc
        Router->>DB: assigned_to = HR
    end

    System->>DB: Tạo Ticket(status=new, assigned_to=...)
    System->>Assignee: Thông báo có ticket mới

    alt Sai bộ phận
        Assignee->>System: Forward sang bộ phận đúng
        System->>DB: Cập nhật assigned_to mới (giữ status=new)
    else Tiếp nhận
        Assignee->>System: Đổi status = processing
        Assignee->>System: Xử lý xong → status = resolved
        Employee->>System: Xác nhận → status = closed
    end

    Note over System: Khiếu nại lương: chỉ trong<br/>3 ngày làm việc từ ngày công bố
```

---

## 6. Vòng Đời Trạng Thái Các Đối Tượng

### Đơn Nghỉ Phép / Tăng Ca

```mermaid
stateDiagram-v2
    [*] --> pending : NV nộp đơn
    pending --> leader_approved : L1 Duyệt
    pending --> rejected : L1 Từ chối
    leader_approved --> approved : HR (L2) Duyệt
    leader_approved --> rejected : HR (L2) Từ chối
    approved --> [*]
    rejected --> [*]
```

### Tài Khoản

```mermaid
stateDiagram-v2
    [*] --> active : HR tạo tài khoản
    active --> locked : Sai MK 3 lần
    active --> locked : NV chuyển Nghỉ việc
    locked --> active : HR/Admin mở khóa
    active --> inactive : Admin vô hiệu hóa
    inactive --> [*]
```

### Ticket Helpdesk

```mermaid
stateDiagram-v2
    [*] --> new : NV tạo ticket
    new --> processing : Người xử lý tiếp nhận
    new --> new : Forward sang bộ phận khác
    processing --> resolved : Đã xử lý xong
    resolved --> closed : NV xác nhận
    new --> rejected : Từ chối ngay
    closed --> [*]
    rejected --> [*]
```

### Hợp Đồng

```mermaid
stateDiagram-v2
    [*] --> active : HR tạo HĐ mới
    active --> warning_30d : Còn 30 ngày
    warning_30d --> warning_15d : Còn 15 ngày
    warning_15d --> warning_7d : Còn 7 ngày
    warning_7d --> expired : Hết hạn chưa gia hạn
    warning_30d --> renewed : HR gia hạn
    warning_15d --> renewed : HR gia hạn
    warning_7d --> renewed : HR gia hạn
    expired --> [*]
    renewed --> active : HĐ mới hiệu lực
```

### Đánh Giá Nhân Viên

```mermaid
stateDiagram-v2
    [*] --> draft : Leader/Manager tạo
    draft --> submitted : Nhấn Gửi (🔒 Khoá chỉnh sửa)
    submitted --> acknowledged : HR xác nhận
    acknowledged --> [*]
```

---

## 7. Quy Định Nghiệp Vụ Quan Trọng

| Mã | Nội dung |
|----|---------|
| `QĐ_TK1` | Sai MK 3 lần → khóa `is_active=False` tự động |
| `QĐ_TK2` | HR mở khóa Employee/Leader/Manager; Admin mở khóa mọi tài khoản |
| `QĐ_Tao_MSNV` | `[YY][MaPhongBan][STT4digit]` — VD: `26IT0001` |
| `QĐ_Tao_Username` | `[ten][ho][maphongban]` viết thường không dấu — VD: `annguyenit` |
| `QĐ_PheDuyet_L1` | Ai là `leader_user` HOẶC `manager_user` của NV trong EmployeeWorkInfo → người đó duyệt L1, không phân biệt số ngày/giờ |
| `QĐ_PheDuyet_L2` | Sau L1 → HR xác nhận tất cả trường hợp |
| `QĐ_CapNhat_DuLieu` | Nghỉ phép, OT, thưởng/phạt chỉ hiệu lực sau khi L2 hoàn tất |
| `QĐ_CanhBao` | 30 ngày (HR+Manager) → 15 ngày (+NV) → 7 ngày (tất cả, KHẨN) |
| `QĐ_DieuChinh` | HR chỉ sửa giờ công trong kỳ hiện tại (chưa chốt lương) |
| `QĐ_LuuTruDanhGia` | Sau khi `submitted` → không thể sửa đánh giá |
| `QĐ_DieuHuong` | Ticket định tuyến tự động theo loại yêu cầu |
| `QĐ_KhieuNai_Luong` | Chỉ trong 3 ngày làm việc từ ngày công bố bảng lương |
| `QĐ_XacNhanBaoCao` | Sau khi `status = acknowledged` → khóa chỉnh sửa báo cáo |
| `QĐ_NghiViec` | NV chuyển trạng thái `resigned` → `is_active=False` tự động |
| `QĐ_Session` | Không hoạt động **30 phút** → tự động đăng xuất |

---

## 8. Tech Checklist Deploy

- [ ] InsightFace cài đúng version, test nhận diện với webcam thực
- [ ] Batch Job cảnh báo HĐ chạy đúng giờ (cron/Celery beat)
- [ ] Gmail SMTP config cho OTP reset mật khẩu
- [ ] Session timeout **30 phút** hoạt động
- [ ] File upload: validate định dạng (PDF/JPG/PNG) và giới hạn 5MB
- [ ] IP whitelist cho chấm công đã cấu hình
- [ ] RBAC đã test đủ 5 role không bị bypass
- [ ] SQLite → cân nhắc chuyển MySQL khi deploy production
- [ ] HTTPS bắt buộc (dữ liệu nhân sự nhạy cảm)

---

> 📌 **Ghi chú:** File này phản ánh codebase tính đến ngày 01/06/2026.  
> Tham chiếu thêm tại [HRMS_CODE_GUIDE.md](file:///d:/Study/Nhập%20môn%20công%20nghệ%20phần%20mềm/Business_web_project/HRMS_CODE_GUIDE.md) để tra cứu mã `QĐ_xxx` và `BM_xx`.
