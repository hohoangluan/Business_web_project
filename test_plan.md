...
| Thứ tự ngày HĐ | ~~Không enforce BĐ≥ký, hết hạn≥BĐ~~ | FUNC-CON-004/005 | ✅ **ĐÃ LÀM** — `validate_contract_date_order` (contracts.services) áp form + create view. Test `test_date_order` |
| Duyệt 2 cấp thưởng/phạt | ~~Chỉ 1 bước pending→approved~~ | FUNC-RW-005 | ✅ **ĐÃ LÀM** — status `leader_approved` + routing theo role; Manager L1, HR L2. Migration 0003. Test 2-level |
| SECRET_KEY | ~~Key mặc định yếu (W009)~~ | SEC-014/015 | ✅ **ĐÃ LÀM** — settings raise `ImproperlyConfigured` khi DEBUG=False + key yếu |
| Auto hết hiệu lực HĐ | ~~HĐ quá hạn không tự `is_active=False`~~ | FUNC-CON-008 | ✅ **ĐÃ LÀM** — `expire_overdue_contracts` gọi trong command. ⚠️ Render free không cron → cần trigger định kỳ |
